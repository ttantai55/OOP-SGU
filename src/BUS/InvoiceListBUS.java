...
public class InvoiceListBUS {
    ...
    public void inputInvoice() {
        ...
        while (themTiep.equals("y")) {
            ...
            WarrantyDTO warranty = new WarrantyDTO(warrantyId, invoice.getInvoiceId(), product, startDate, endDate,
                    true);
    ...
    public void printInvoicesByCustomer() {
        ...
        if (results == null || results.length == 0) {
            System.out.println("[Thong bao] Khong tim thay hoa don nao cua khach hang: " + customerName);
            return;
        }
    ...
    public void checkoutFromCart(String username, CartItemDTO[] myCart, CartBUS cartBUS) {
        ...
        if (payment != null) {
            ...
            for (InvoiceItemDTO item : invoice.getInvoiceItemList()) {
                invItemDAO.add(item);
                // productBus.deductStock(item.getProductId(), item.getQuantity()); <-- CODE TRỪ
                // KHO ĐỂ Ở ĐÂY
            }
